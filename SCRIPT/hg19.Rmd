...
```{r}
acmg_list_imp <- readxl::read_xlsx("DATA/acmg_list.xlsx")
acmg_to_vec<-setDT(acmg_list_imp)[, .(Gene)]
acmg_vec <- as.vector(acmg_to_vec)
...
hg19_trim_chrom_1<-hg19_trim_chrom
hg19_trim_chrom_2<- hg19_trim_chrom_1 %>% group_by(name2) %>% top_n(1, exonCount) %>% filter(name2 %in% acmg_to_vec$Gene) %>% ungroup()
hg19_trim_chrom_2$index <- rownames(hg19_trim_chrom_2)
hg19_trim_chrom_2$index <- as.numeric(hg19_trim_chrom_2$index)
# setDT(hg19_trim_chrom_2)
exon_input<- setDT(hg19_trim_chrom_2)
###########original exon input ########
...
for (i in exon_input$index) {
  ...
  exons.df$tx_start<-unit$txStart
  exons.df$tx_end<-unit$txEnd
...
exons_strand <- exons_init %>%
  ...
  mutate(start_tx = ifelse(strand == "-", paste0(tx_end), paste0(tx_start)))%>%
  mutate(end_tx = ifelse(strand == "-", paste0(tx_start), paste0(tx_end)))
...
plus_box$cds_exon_start <- as.numeric(plus_box$cds_exon_start)
plus_box$cds_exon_end <- as.numeric(plus_box$cds_exon_end)
plus_box_1 <- plus_box %>% mutate(intron_start = paste(cds_exon_end+1)) %>% mutate(intron_end = paste(cds_exon_start-1))
plus_box_2<- plus_box_1 %>% mutate(intron = paste(exon -1))
...
minus_box_arr$cds_exon_start <- as.numeric(minus_box_arr$cds_exon_start)
minus_box_arr$cds_exon_end <- as.numeric(minus_box_arr$cds_exon_end)
minus_box_arr_1 <- minus_box_arr %>% mutate(intron_start = paste(cds_exon_start+1)) %>% mutate(intron_end = paste(cds_exon_end-1))
minus_box_arr_2<- minus_box_arr_1 %>% mutate(intron = paste(exon -1))
...
both_sets <- rbind(plus_box_2, minus_box_arr_2)
#both_sets_tst <- both_sets %>% select(-exon) ############# tons of NAs
## difs
#ummm <- setdiff(exons_strand_all, both_sets)

str(both_sets)
############## get rid of old column3###############
setDT(both_sets)[, start_tx := as.numeric(start_tx)][, end_tx := as.numeric(end_tx)][, coding_start := as.numeric(coding_start)][, coding_end := as.numeric(coding_end)]
both_sets_all_products <- both_sets %>% select(-exon_start, -exon_end, -cds_start, -cds_end) %>% mutate(tx_length = abs(start_tx - end_tx)) %>% mutate(coding_length = abs(coding_start - coding_end)) %>% mutate(cds_length = abs(cds_exon_start-cds_exon_end)) %>% mutate(genomic_coords = paste(chr ,start_tx, sep = ":")) %>% mutate(gen_coord = paste(genomic_coords, end_tx, sep = ))
...
```{r}
longest_exons<- for_igv_all  %>% filter(max(exon_count))
library(seqinr)
myfile <-system.file("DATA/sequence.fasta", package = "seqinr")
lmna_fas_1 <- read.fasta(file= myfile)
lmna_fas<-read.fasta(file = system.file("DATA/sequence.fasta", package = "seqinr"), seqtype = "DNA", as.string = F, set.attributes = T, strip.desc = T, whole.header = F)

lmna<-read_file("DATA/lmna")
lmna_c<-  unlist(strsplit(lmna, "\\b"))
```



```{r}
#################reproduce gene plots ###############

b1_b2<- for_igv_all %>% filter(symbol %in% c("BRCA1","BRCA2"))%>% filter(name == c("NM_007294.4", "NM_000059.4"))
b1_b2_strand <- b1_b2 %>% mutate(strands = ifelse(strand == "+", 1, 0)) %>% select(-strand) %>% rename(strand=strands)
#fwrite(b1_b2_strand, file = "OUTPUT/b1_b2_out.csv")

# b1_b2_1 <- b1_b2 %>% mutate(plot_start = cds_exon_start +)
p1<- ggplot(data = b1_b2, y=symbol)+
    geom_segment( aes(x=tx_start, xend= tx_end, y=0.5, yend=0.5),size=10,color='#a7a9ac')


p2 <- p1 + geom_rect(data = b1_b2, mapping = aes(xmin=cds_exon_start, xmax=cds_exon_end, ymin=0.45, ymax=0.55),
color = "black",
fill = "#56B4E9")+
...
```{r}
#########TESTING INTRONS FOR MINUS STRAND###################
###startr with just brca#######

intron_init <- minus_box_arr
str(intron_init)
intron_brca<- intron_init %>% filter(symbol == "BRCA1") %>% filter(name == "NM_007294.4", "NM_000059.4")
intron_brca$cds_exon_start <- as.numeric(intron_brca$cds_exon_start)
intron_brca$cds_exon_end <- as.numeric(intron_brca$cds_exon_end)
str(intron_brca)
########cds_exon_start 
intron_bound_1 <- intron_brca %>% mutate(intron_start = paste(cds_exon_start+1)) %>% mutate(intron_end = paste(cds_exon_end-1))
intron_numbers<- intron_bound_1 %>% mutate(intron = paste(exon -1))
```
